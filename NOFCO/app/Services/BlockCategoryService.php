...
class BlockCategoryService
{
    ...
    function create($dataIn = [], BlockCategory &$BlockCategory)
    {

        if (array_key_exists('image', $dataIn)) {
            $dataIn['image'] = $dataIn['image']->store('uploads');
        }
        $this->mapDataModel($dataIn, $BlockCategory);
        $BlockCategory->save();
    }

    public function mapDataModel($data, BlockCategory &$model)
    {
        $attribute = [
            'record_order',
            'name',
    ...
    function update($dataIn = [], BlockCategory &$BlockCategory)
    {
        if (array_key_exists('image', $dataIn)) {
            $dataIn['image'] = $dataIn['image']->store('uploads');
        }
    ...
    {
        if ($id > 0) {
            $res = BlockCategory::findOrFail($id);
            return $res;
        } else {
